# SpireScope 一般プレイヤー向け配布計画

Slay the Spire 2 をインストール済みのプレイヤーが、**開発環境なし・ゲームアセットの再配布なし**で
SpireScope（セーブデータビューア）を利用できるようにするための計画書。
...
SpireScope は現在、開発者本人の環境でのみ動作する。他のプレイヤーに使ってもらうには
...
| 論点 | 決定 | 理由 |
|---|---|---|
| 配布対象 | **SpireScope のみ** | 一般プレイヤーに価値があるのはセーブビューア。他は開発者用 |
...
| 課題 | 現状 |
|---|---|
| アセット解決 | exe の親を遡って `tools/extracted` を探すウォークアップ方式が約 8 箇所に分散（`SpireScope/DeckOverviewForm.cs:674`、`SpireScope/Services/RelicImageService.cs:89`、`CardAtlasService.cs` / `EnchantmentIconService.cs` / `MonsterImageService.cs`、`StS2Capture.Core` の認識系 4 ファイル）。配布 exe の親に `tools/extracted` は存在しないため画像が出ない |
...
2. **配布モード**: `%LocalAppData%\SpireScope\assets\{gameVersion}\` を探す
3. どちらも無ければ「未セットアップ」を返し、呼び出し側（SpireScope）がセットアップウィザードを起動
...
### Phase 3: 初回セットアップウィザード（SpireScope 内蔵）

起動時に `AssetLocator` が「未セットアップ」を返したら表示するモーダルフォーム。

**抽出対象マニフェスト**（SpireScope が実行時に参照するもののみ。全展開はしない）:
...
2. 抽出内容と保存先（`%LocalAppData%\SpireScope\assets\{gameVersion}\`）の確認表示
...
```powershell
dotnet publish SpireScope -c Release -r win-x64 --self-contained `
...
1. GitHub Releases から `SpireScope-vX.Y.Z-win-x64.zip` をダウンロードして展開
2. `SpireScope.exe` を起動（SmartScreen 警告が出た場合は「詳細情報 → 実行」— README に手順記載）
