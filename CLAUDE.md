...
# 5. モンスター画像（Spine → PNG/GIF）。--dist-dir は必ずこのリポジトリ配下を指定する
dotnet run --project StS2SiteBuilder -- --build --dist-dir .\StS2SiteBuilder\dist
```
...
- `images/monsters/{id}.png` `{id}.gif` は **SiteBuilder ビルドの生成物**（`GenerateMonsterGifs`）。
